...
public:
	...
	real surface_r = 1.4f;
	real beta = 50.0;//used for feedback
	...
	virtual void Advance(const real dt)
	{
		...
		for (int j = 0; j < 3; j++) {
			...
			for (int i = 0; i < new_particles_H.Size(); i++) {
				new_particles_H.V(i) += new_particles_H.F(i) / new_particles_H.D(i)*dt / 3;
				new_particles_H.X(i) += new_particles_H.V(i)*dt / 3;
			}
		}
