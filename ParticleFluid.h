...
public:
	...
	real threshold_new = .43; // threshold determining high resolution region
	real threshold_old = .2; // threshold for existing high resolution particles
	real surface_r = 1.4f;
	...
	void Update_H()
	{
		...
		std::cout << "h1_count: " << h1_count << std::endl;
	...
	void Update_boundary_region()
	{
			...
			for (int i = 0; i < particles.Size(); i++)
			{
				...
				for (int j = 0; j < surface_neighbors[i].size(); j++)
				{
					if (particles.H(surface_neighbors[i][j]) == 0 && surface_neighbors[i][j]!=i)//if the particle has a low resolution neighbor, then set it to bondary
					{
						flag_l = true;
					...
					if (particles.H(surface_neighbors[i][j]) == 1)
					{
						flag_h = true;
					}
					if (flag_l && flag_h)
		...
		//std::cout << "h2 count: " << boundary_idx.size() << std::endl;
		//std::cout << "expected h1 count: " << parents_idx.size() - boundary_idx.size() << std::endl;
		//std::cout << "parent count:" << parents_idx.size() << std::endl;
